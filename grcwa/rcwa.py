...
class obj:
    ...
    def Init_Setup(self, Pscale: float = 1.0, Gmethod: int = 0) -> None:
        ...
        kx0 = (
            ...
            * bd.sin(self.theta)
            * bd.cos(self.phi)
            * bd.sqrt(self.Uniform_ep_list[0])
        )
        ky0 = (
            self.omega
            * bd.sin(self.theta)
    ...
    def MakeExcitationPlanewave(
        ...
        if direction == "forward":
            ...
            tmp1[order] = 1.0
            a0 = a0 + tmp1 * (
                -s_amp * bd.cos(theta) * bd.cos(phi) * bd.exp(1j * s_phase)
            ...
            a0 = a0 + tmp2 * (
                -s_amp * bd.cos(theta) * bd.sin(phi) * bd.exp(1j * s_phase)
    ...
    def GridLayer_geteps(self, ep_all) -> None:
        ...
        for i in range(self.Layer_N):
            ...
            if len(ep_all) == 3 and ep_all[0].ndim > 0:
                ep_grid = [
                    bd.reshape(ep_all[0][ptr : ptr + Nx * Ny], [Nx, Ny]),
    ...
    def Volume_integral(self, which_layer, Mx, My, Mz, normalize=0):
        ...
        # F matrix
        Faxy = bd.dot(bd.dot(kp, phi), bd.diag(1.0 / self.omega / q))
        Faz1 = 1.0 / self.omega * bd.dot(epinv, bd.diag(self.ky))
        Faz2 = -1.0 / self.omega * bd.dot(epinv, bd.diag(self.kx))
...
def GetSMatrix(indi, indj, q_list, phi_list, kp_list, thickness_list):
    ...
    for l in range(indi, indj):
        ...
        P2 = bd.dot(bd.dot(kp_list[lp1], phi_list[lp1]), bd.diag(1.0 / q_list[lp1]))
        P = bd.dot(P1, P2)
...
def GetZPoyntingFlux(ai, bi, omega, kp, phi, q, byorder=0):
    ...
    A = bd.dot(bd.dot(kp, phi), bd.diag(1.0 / omega / q))
...
def Matrix_zintegral(q, thickness, shift=1e-12):
    ...
    Mab = (
        (bd.exp(1j * qj * thickness) - bd.exp(-1j * bd.conj(qi) * thickness))
        / 1j
        / qij2
    )
