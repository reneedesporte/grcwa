...
if AG_AVAILABLE:
    ...
    def test_thickgrad():
        def fun(x):
            obj = rcwa_assembly(
                epgrid.flatten(), freq, theta, phi, planewave, x, Pscale=1.0
            )
            R, T = obj.RT_Solve(normalize=1)
            return R
    ...
    def test_periodgrad():
        def fun(x):
            obj = rcwa_assembly(
                epgrid.flatten(), freq, theta, phi, planewave, pthick, Pscale=x
            )
            R, T = obj.RT_Solve(normalize=1)
            return R
    ...
    def test_freqgrad():
        def fun(x):
            obj = rcwa_assembly(
                epgrid.flatten(), x, theta, phi, planewave, pthick, Pscale=1.0
            )
            R, T = obj.RT_Solve(normalize=1)
            return R
    ...
    def test_thetagrad():
        def fun(x):
            obj = rcwa_assembly(
                epgrid.flatten(), freq, x, phi, planewave, pthick, Pscale=1.0
            )
            R, T = obj.RT_Solve(normalize=1)
            return R
